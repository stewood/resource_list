...
This script imports coverage areas and resource coverage relationships in batches.
"""
...
def migrate_coverage_areas_batch(batch_size=100):
    """Migrate coverage areas from JSON export in batches."""
    
    print("🔄 Migrating coverage areas in batches...")
    ...
    default_user, created = User.objects.get_or_create(
        username="gis_migrator_batch",
        defaults={
            "email": "gis_migrator_batch@example.com",
            "first_name": "GIS",
            "last_name": "Migrator Batch",
        }
    )
    ...
    # Process in batches
    total_batches = (len(coverage_data) + batch_size - 1) // batch_size
    
    for batch_num in range(total_batches):
        start_idx = batch_num * batch_size
        end_idx = min(start_idx + batch_size, len(coverage_data))
        batch_data = coverage_data[start_idx:end_idx]
        ...
        print(f"   ✅ Batch completed: {created_count} created, {skipped_count} skipped, {error_count} errors")
    
    print(f"\n✅ Coverage areas migration completed!")
...
def migrate_resource_coverage(coverage_map):
    ...
    print(f"✅ Resource coverage migration completed!")
    print(f"   - Created: {created_count}")
    print(f"   - Skipped (already exist): {skipped_count}")
    print(f"   - Errors: {error_count}")
...
def main():
    ...
    print("🚀 Starting GIS data migration (batch mode)...")
    print("=" * 50)
    
    # Step 1: Migrate coverage areas in batches
    coverage_map = migrate_coverage_areas_batch(batch_size=100)
    if not coverage_map:
