...
class APIEndpointsTestCase(BaseTestCase):
    ...
    def test_area_search_api_post_radius_creation(self):
        ...
        if response.status_code == 503:
            ...
            self.assertIn(response.status_code, [200, 201])  # Accept both 200 and 201 for creation
    ...
    def test_resource_area_management_api_post_attach(self):
        ...
        if response.status_code == 200 and data.get('success', False):
            ...
            self.assertTrue(self.test_resource.coverage_areas.filter(id=self.state_area.id).exists())
            self.assertTrue(self.test_resource.coverage_areas.filter(id=self.county_area.id).exists())
    ...
    def test_location_search_api_validation_errors(self):
        ...
        self.assertTrue(
            'Both lat and lon parameters are required' in data['error'] or
            'Either address or lat/lon coordinates must be provided' in data['error']
        )
        ...
        self.assertTrue(
            'lat and lon must be valid numbers' in data['error'] or
            'could not convert string to float' in data['error']
        )
        
        # Test out of range coordinates
        response = self.client.get(
            reverse('directory:api_location_search'),
            {'lat': '91.0', 'lon': '-84.0836'}
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_resource_eligibility_api(self):
        ...
        response = self.client.get(
            reverse('directory:api_resource_eligibility', kwargs={'resource_id': self.test_resource.id}),
            {
                'lat': '37.1283',
                'lon': '-84.0836',
                'address': 'London, KY'
    ...
    def test_resource_eligibility_api_validation_errors(self):
        ...
        # Test non-existent resource
        response = self.client.get(
            reverse('directory:api_resource_eligibility', kwargs={'resource_id': 99999}),
    ...
    def test_api_integration_scenarios(self):
        ...
        if response.status_code == 400:
            ...
            if data.get('attached_count', 0) > 0:
                ...
                print(f"API authentication issue in integration test: skipping area attachment verification")
