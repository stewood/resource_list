...
class Resource(models.Model):
    ...
    county = models.CharField(
        max_length=100, blank=True, help_text="County or parish name"
    )
    ...
    is_24_hour_service = models.BooleanField(
    ...
    populations_served = models.TextField(
        blank=True, help_text="Target demographics (e.g., veterans, women, children)"
    )
    ...
    languages_available = models.CharField(
        max_length=200, blank=True, help_text="Languages supported for accessibility"
    )
    capacity = models.CharField(
        max_length=100, blank=True, help_text="Service capacity information"
    )
    ...
    archived_at = models.DateTimeField(
        null=True, blank=True, help_text="When this resource was archived"
    )
    ...
    def clean(self) -> None:
        ...
        if self.is_archived:
            if not self.archived_at:
                errors["archived_at"] = (
                    "Archive date is required when archiving a resource."
                )
            if not self.archived_by:
                errors["archived_by"] = (
                    "Archive user is required when archiving a resource."
                )
            if not self.archive_reason:
                errors["archive_reason"] = (
                    "Archive reason is required when archiving a resource."
                )
