...
class ResourceManager(models.Manager):
    ...
    def filter_by_location(
        ...
        if not getattr(settings, "GIS_ENABLED", False):
            ...
            logger.warning(
                "GIS not enabled, falling back to text-based location matching"
            )
        ...
        try:
            ...
            if include_radius_search and radius_miles:
                ...
                radius_resources = self.filter(coverage_areas__isnull=True).filter(
    ...
    def _geocode_location(
        self, address: str, provider_name: Optional[str] = None
        ...
        try:
            ...
            if result and result.is_valid():
                logger.info(
                    f"Successfully geocoded '{address}' to ({result.latitude}, {result.longitude})"
                )
    ...
    def _filter_by_location_fallback(
        ...
        ).exclude(Q(city="") & Q(state="") & Q(postal_code=""))

    def annotate_coverage_specificity(
        ...
        try:
            ...
            specificity_case = Case(
                When(
                    coverage_areas__kind="RADIUS",
                    then=Value(specificity_scores["RADIUS"]),
                ),
                When(
                    coverage_areas__kind="POLYGON",
                    then=Value(specificity_scores["POLYGON"]),
                ),
                When(
                    coverage_areas__kind="CITY", then=Value(specificity_scores["CITY"])
                ),
                When(
                    coverage_areas__kind="COUNTY",
                    then=Value(specificity_scores["COUNTY"]),
                ),
                When(
                    coverage_areas__kind="STATE",
                    then=Value(specificity_scores["STATE"]),
                ),
                default=Value(0),
    ...
    def _annotate_coverage_specificity(
        self, queryset: models.QuerySet, search_point: Any
    ) -> models.QuerySet:
    ...
    def annotate_proximity_ranking(
        self, queryset: models.QuerySet, lat: float, lon: float
    ) -> models.QuerySet:
        ...
        try:
            ...
            queryset = queryset.annotate(
                ...
                proximity_score=models.ExpressionWrapper(
                    ...
                    * (1.0 / (1.0 + models.F("distance_miles"))),
                    output_field=models.FloatField(),
                ),
            )
    ...
    def filter_by_location_with_proximity(
        ...
        if not getattr(settings, "GIS_ENABLED", False):
            ...
            logger.warning(
                "GIS not enabled, using basic location filtering without proximity ranking"
            )
        ...
        if sort_by_proximity:
            queryset = queryset.order_by(
                "-proximity_score", "-specificity_score", "distance_miles"
            )

        return queryset

    def filter_by_coverage_area(
        self, coverage_area_id: int, include_children: bool = False
    ) -> models.QuerySet:
    ...
    def get_coverage_statistics(self) -> Dict[str, Any]:
        ...
        coverage_types = (
            self.filter(coverage_areas__isnull=False)
            .values("coverage_areas__kind")
        ...
        return {
            ...
            "coverage_percentage": (
                (with_coverage / total_resources * 100) if total_resources > 0 else 0
            ),
            "coverage_types": list(coverage_types),
        }

    def calculate_resource_distance(
        self, resource_id: int, lat: float, lon: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not getattr(settings, "GIS_ENABLED", False):
                result["eligibility_reason"] = (
                    "GIS not enabled for distance calculations"
                )
                return result
            ...
            for coverage_area in coverage_areas:
                if coverage_area.geom:
                    ...
                    if coverage_area.geom.contains(query_point):
                        ...
                        result["eligibility_reason"] = (
                            f"Location is within {coverage_area.name} ({coverage_area.kind})"
                        )
            ...
            if not result["serves_location"] and result["coverage_areas"]:
                closest_area = min(
                    result["coverage_areas"], key=lambda x: x["distance_miles"]
                )
                result["eligibility_reason"] = (
                    f'Location not served. Closest area: {closest_area["name"]} ({closest_area["distance_miles"]} miles away)'
                )
    ...
    def check_location_eligibility(
        ...
        try:
            ...
            for resource in nearby_resources:
                ...
                distance_info = self.calculate_resource_distance(resource.id, lat, lon)
            ...
            return {"error": str(e), "serving_resources": [], "nearby_resources": []}
