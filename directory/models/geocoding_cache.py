...
class GeocodingCache(models.Model):
    ...
    query = models.TextField(
        help_text="Original geocoding query string"
    )
    ...
    provider = models.CharField(
        max_length=50,
        help_text="Name of the geocoding provider used"
    )
    ...
    def __str__(self) -> str:
        return f"GeocodingCache({self.query[:50]}... -> {self.latitude}, {self.longitude})"
    ...
    @classmethod
    def get_cached_result(cls, query: str, provider: Optional[str] = None) -> Optional['GeocodingCache']:
    ...
    def cleanup_old_entries(cls, days_old: int = 7) -> int:
        ...
        old_count, _ = cls.objects.filter(
    ...
    def get_cache_stats(cls) -> dict:
        ...
        provider_stats = cls.objects.values('provider').annotate(
            count=models.Count('id')
        ).order_by('-count')
        
        # Get average hit count
        avg_hit_count = cls.objects.aggregate(
