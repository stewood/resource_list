...
class ResourceForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'last_verified_at': forms.DateTimeInput(attrs={
                'class': 'form-control',
                'type': 'datetime-local'
            }),
            'last_verified_by': forms.Select(attrs={
                'class': 'form-control'
            }),
        }
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if 'last_verified_by' in self.fields:
            self.fields['last_verified_by'].queryset = User.objects.filter(is_staff=True)
    ...
    def clean(self) -> Dict[str, Any]:
        ...
        if status == 'draft':
            ...
            if not any([phone, email, website]):
                raise ValidationError("At least one contact method (phone, email, or website) is required.")
        ...
        # Needs review validation
            ...
            if not description or len(description) < 20:
                raise ValidationError("Description must be at least 20 characters for review.")
        ...
        # Published validation
            ...
            if not last_verified_at:
                raise ValidationError("Verification date is required for published resources.")
            ...
            if last_verified_at:
                ...
                expiry_date = last_verified_at + timedelta(days=settings.VERIFICATION_EXPIRY_DAYS)
                if timezone.now() > expiry_date:
                    raise ValidationError(f"Verification must be within {settings.VERIFICATION_EXPIRY_DAYS} days.")
