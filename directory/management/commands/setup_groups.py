...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._assign_permissions(
            editor_group, editor_permissions, resource_ct, category_ct, user_ct
        )

        self.stdout.write("\nAssigning permissions to Reviewer group:")
        self._assign_permissions(
            reviewer_group, reviewer_permissions, resource_ct, category_ct, user_ct
        )

        self.stdout.write("\nAssigning permissions to Admin group:")
        self._assign_permissions(
            admin_group, admin_permissions, resource_ct, category_ct, user_ct
        )
    ...
    def _assign_permissions(
        ...
        for perm_name in permission_names:
            ...
            if (
                ...
                or perm_name.startswith("change_")
                or perm_name.startswith("delete_")
                or perm_name.startswith("view_")
            ):
