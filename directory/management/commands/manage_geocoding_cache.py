...
class Command(BaseCommand):
    ...
    def _cleanup_expired(self, dry_run: bool):
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would remove {expired_count} expired entries')
            )
        else:
    ...
    def _cleanup_old(self, days: int, dry_run: bool):
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would remove {old_count} entries older than {days} days')
            )
        else:
            old_count = GeocodingCache.cleanup_old_entries(days)
            self.stdout.write(
                self.style.SUCCESS(f'Removed {old_count} cache entries older than {days} days')
            )
    ...
    def _clear_all(self, dry_run: bool):
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would remove all {total_count} cache entries')
            )
        else:
            # Confirm with user
            total_count = GeocodingCache.objects.count()
            confirm = input(f'Are you sure you want to delete all {total_count} cache entries? (yes/no): ')
