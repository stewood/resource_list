...
class Command(BaseCommand):
    ...
    def _display_summary(self, report: Dict[str, Any], min_quality_score: float) -> None:
        ...
        if total_errors > 0:
            self.stdout.write("\n📋 Issue Breakdown:")
            self.stdout.write(f"  • FIPS Code Errors: {summary.get('fips_errors_count', 0)}")
            ...
            self.stdout.write(f"  • Spatial Issues: {summary.get('spatial_issues_count', 0)}")
    ...
    def _export_report(self, report: Dict[str, Any], filename: str) -> None:
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Error exporting report: {str(e)}")
            )
    ...
    def _fix_issues(self, report: Dict[str, Any]) -> None:
        ...
        if report.get('name_issues'):
            self.stdout.write("  • Name consistency issues can be fixed with data cleanup")
            fixes_applied += len(report['name_issues'])
        
        if report.get('spatial_issues'):
            self.stdout.write("  • Spatial integrity issues can be fixed with geometry repair")
            fixes_applied += len(report['spatial_issues'])
        
        if fixes_applied > 0:
            self.stdout.write(
                self.style.SUCCESS(f"  • {fixes_applied} issues identified for automatic fixing")
            )
        else:
