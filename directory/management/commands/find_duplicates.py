...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 Starting duplicate detection..."))
    ...
    def print_summary(self, summary: Dict[str, int]):
        ...
        self.stdout.write(
            f"📝 Exact name duplicates: {summary['exact_name_groups']} groups"
        )
    ...
    def print_exact_duplicates(
        self, duplicates: Dict[str, List[Resource]], show_details: bool
    ):
        ...
        self.stdout.write("\n" + "-" * 60)
        self.stdout.write(
            self.style.WARNING("🔴 HIGH CONFIDENCE: Exact Name Duplicates")
        )
        self.stdout.write("-" * 60)
        ...
        for name, resources in duplicates.items():
            ...
            for resource in resources:
                self.stdout.write(
                    f"   - ID: {resource.id} | Status: {resource.status} | Created: {resource.created_at.date()}"
                )
                if show_details:
                    self.stdout.write(f"     Phone: {resource.phone or 'N/A'}")
                    self.stdout.write(f"     Email: {resource.email or 'N/A'}")
                    self.stdout.write(
                        f"     Address: {resource.address1 or 'N/A'}, {resource.city or 'N/A'}, {resource.state or 'N/A'}"
                    )

    def print_contact_duplicates(
        self, duplicates: List[Tuple[Resource, Resource, str]], show_details: bool
    ):
        ...
        self.stdout.write("\n" + "-" * 60)
        self.stdout.write(
            self.style.WARNING("🔴 HIGH CONFIDENCE: Contact Information Duplicates")
        )
        self.stdout.write("-" * 60)
    ...
    def print_phone_duplicates(
        ...
        self.stdout.write("\n" + "-" * 60)
        self.stdout.write(
            self.style.WARNING("🟡 MEDIUM CONFIDENCE: Phone Number Duplicates")
        )
        self.stdout.write("-" * 60)
        ...
        for phone, resources in duplicates.items():
            ...
            for resource in resources:
                ...
                if show_details:
                    self.stdout.write(
                        f"     Status: {resource.status} | Created: {resource.created_at.date()}"
                    )

    def print_website_duplicates(
        self, duplicates: Dict[str, List[Resource]], show_details: bool
    ):
        ...
        self.stdout.write("\n" + "-" * 60)
        self.stdout.write(
            self.style.WARNING("🟡 MEDIUM CONFIDENCE: Website Duplicates")
        )
        self.stdout.write("-" * 60)
    ...
    def print_fuzzy_duplicates(
        self, duplicates: List[Tuple[Resource, Resource, float]], show_details: bool
    ):
        ...
        self.stdout.write("\n" + "-" * 60)
        self.stdout.write(
            self.style.WARNING("🟢 LOW CONFIDENCE: Fuzzy Name Duplicates")
        )
        self.stdout.write("-" * 60)
