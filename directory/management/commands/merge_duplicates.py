...
class DuplicateMerger:
        ...
        try:
            with transaction.atomic():
                ...
                if not duplicates.exists():
                    raise CommandError(f"No duplicate resources found with IDs: {duplicate_ids}")
                ...
                archived_resources = self._archive_duplicates(duplicates, primary_id, merge_notes)
    ...
    def _create_backup_version(self, resource: Resource, change_type: str) -> None:
        ...
        ResourceVersion.objects.create(
            resource=resource,
            version_number=ResourceVersion.objects.filter(resource=resource).count() + 1,
    ...
    def _merge_resource_data(self, primary: Resource, duplicates: List[Resource]) -> Dict[str, Any]:
        ...
        for resource in all_resources:
            ...
            if resource.description and resource.description not in merged_data['descriptions']:
            ...
            if resource.hours_of_operation and resource.hours_of_operation not in merged_data['hours']:
                merged_data['hours'].append(resource.hours_of_operation)
            if resource.eligibility_requirements and resource.eligibility_requirements not in merged_data['eligibility']:
                merged_data['eligibility'].append(resource.eligibility_requirements)
            if resource.populations_served and resource.populations_served not in merged_data['populations']:
                merged_data['populations'].append(resource.populations_served)
            if resource.insurance_accepted and resource.insurance_accepted not in merged_data['insurance']:
                merged_data['insurance'].append(resource.insurance_accepted)
            if resource.cost_information and resource.cost_information not in merged_data['costs']:
                merged_data['costs'].append(resource.cost_information)
            if resource.languages_available and resource.languages_available not in merged_data['languages']:
    ...
    def _update_primary_resource(self, primary: Resource, merged_data: Dict[str, Any]) -> None:
        """Update the primary resource with merged data."""
    ...
    def _archive_duplicates(self, duplicates: List[Resource], primary_id: int, merge_notes: str) -> List[Resource]:
        ...
        for duplicate in duplicates:
            ...
            duplicate.archive_reason = f"Merged into primary resource ID {primary_id}. {merge_notes}"
    ...
    def _log_merge_operation(self, primary: Resource, duplicates: List[Resource], merge_notes: str) -> None:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['auto_merge']:
            ...
            self.stdout.write(
                self.style.ERROR('Please specify either --auto-merge or both --primary-id and --duplicate-ids')
            )
    ...
    def _manual_merge(self, options: Dict[str, Any]) -> None:
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🔄 Starting manual merge of {len(duplicate_ids)} duplicates into primary ID {primary_id}')
        )
        ...
        self.stdout.write(f'📊 Archived {len(result["archived_ids"])} duplicate resources')
    ...
    def _auto_merge_duplicates(self, options: Dict[str, Any]) -> None:
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🔄 Starting auto-merge with {confidence} confidence level')
        )
    ...
    def _show_merge_preview(self, primary_id: int, duplicate_ids: List[int]) -> None:
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Resource not found: {e}')
            )
