...
class GeometryProcessor:
    ...
    MIN_AREA_SQ_DEGREES = (
        1e-10  # Minimum area in square degrees (very small but not zero)
    )
    ...
    def simplify_for_display(
        cls, geometry: GEOSGeometry, max_vertices: Optional[int] = None
    ) -> GEOSGeometry:
        ...
        logger.debug(
            f"Simplifying geometry with {current_vertices} vertices to max {max_vertices}"
        )
    ...
    def simplify_for_storage(
        cls, geometry: GEOSGeometry, tolerance: Optional[float] = None
    ) -> GEOSGeometry:
    ...
    def _adaptive_simplify(
        cls, geometry: GEOSGeometry, target_vertices: int
    ) -> GEOSGeometry:
    ...
    def fix_winding_order(cls, geometry: GEOSGeometry) -> GEOSGeometry:
        ...
        if geometry.geom_type == "Polygon":
            ...
            raise ValueError(
                f"Unsupported geometry type for winding order fix: {geometry.geom_type}"
            )
