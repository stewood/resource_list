...
class DuplicateDetector:
    ...
    def normalize_address(self, address: str) -> str:
        ...
        address = re.sub(
            r"\b(st|street|ave|avenue|rd|road|blvd|boulevard|ln|lane|dr|drive)\b",
            "",
            address,
        )
    ...
    def find_exact_duplicates(self) -> Dict[str, List[Resource]]:
        ...
        return {
            name: resources
            for name, resources in name_groups.items()
            if len(resources) > 1
        }
    ...
    def find_phone_duplicates(self) -> Dict[str, List[Resource]]:
        ...
        return {
            phone: resources
            for phone, resources in phone_groups.items()
            if len(resources) > 1
        }
    ...
    def find_website_duplicates(self) -> Dict[str, List[Resource]]:
        ...
        return {
            website: resources
            for website, resources in website_groups.items()
            if len(resources) > 1
        }
    ...
    def find_email_duplicates(self) -> Dict[str, List[Resource]]:
        ...
        return {
            email: resources
            for email, resources in email_groups.items()
            if len(resources) > 1
        }
    ...
    def find_address_duplicates(self) -> Dict[str, List[Resource]]:
        ...
        return {
            address: resources
            for address, resources in address_groups.items()
            if len(resources) > 1
        }
        ...
        for i, resource1 in enumerate(resources_list):
            for j, resource2 in enumerate(resources_list[i + 1 :], i + 1):
                ...
                if (
                    resource1.phone
                    and resource2.phone
                    and self.normalize_phone(resource1.phone)
                    == self.normalize_phone(resource2.phone)
                ):
                ...
                if (
                    resource1.email
                    and resource2.email
                    and resource1.email.lower().strip()
                    == resource2.email.lower().strip()
                ):
