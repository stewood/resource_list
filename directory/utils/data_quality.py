...
class DataQualityChecker:
    ...
    def comprehensive_quality_check(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            report["summary"] = {"error": str(e), "status": "Error"}
        ...
        try:
            ...
            for area in coverage_areas:
                ...
                if area.kind == "CITY":
                    ...
                    if city_fips and not re.match(
                        cls.CITY_FIPS_PATTERN, str(city_fips)
                    ):
                        errors.append(
                            {
                                "area_id": area.id,
                ...
                if area.kind == "COUNTY":
                    ...
                    if state_fips and county_fips:
                        ...
                        existing = CoverageArea.objects.filter(kind="COUNTY").exclude(
                            id=area.id
                        )
                        ...
                        for existing_area in existing:
                            ...
                            if (
                                existing_ext_ids.get("state_fips") == state_fips
                                and existing_ext_ids.get("county_fips") == county_fips
                            ):
        ...
        try:
            ...
            for i, area1 in enumerate(areas_list):
                for j, area2 in enumerate(areas_list[i + 1 :], i + 1):
                    ...
                    if area1.kind != area2.kind and not cls._should_check_overlap(
                        area1, area2
                    ):
                        continue
                        ...
                        try:
                            ...
                            if not intersection.empty:
                                overlap_ratio = intersection.area / min(
                                    area1.geom.area, area2.geom.area
                                )
                    ...
                    name_similarity = cls._calculate_name_similarity(
                        area1.name, area2.name
                    )
                    if (
                        name_similarity > cls.MIN_NAME_SIMILARITY
                        and area1.kind == area2.kind
                    ):
                        duplicates.append(
                            {
                                "type": "name_duplicate",
    ...
    def _should_check_overlap(cls, area1: CoverageArea, area2: CoverageArea) -> bool:
        ...
        return (area1.kind, area2.kind) in overlap_combinations or (
            area2.kind,
            area1.kind,
        ) in overlap_combinations
    ...
    def _generate_recommendations(cls, report: Dict[str, Any]) -> List[str]:
        ...
        if report["fips_errors"]:
            recommendations.append(
                "Fix FIPS code validation errors to ensure data consistency"
            )
        ...
        if report["name_issues"]:
            recommendations.append(
                "Standardize naming conventions across coverage areas"
            )
