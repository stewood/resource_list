#!/usr/bin/env python3
"""
Update Specific Resource Script

This script updates a specific resource by ID using data from a JSON file.
"""
...
def update_resource_from_dict(resource, update_data):
    ...
    if 'website' in update_data:
        ...
        resource.website = update_data['website'] if update_data['website'] else ""
...
def transition_to_review(resource, user):
    """Transition resource to needs_review status."""
    try:
        ...
        print(f"Error transitioning to review: {e}")
        return False


def main():
    parser = argparse.ArgumentParser(description='Update a specific resource by ID')
    parser.add_argument('--resource-id', type=int, required=True, help='Resource ID to update')
    parser.add_argument('--data-file', required=True, help='Path to JSON file with update data')
    parser.add_argument('--user', default='admin', help='Username for the update (default: admin)')
